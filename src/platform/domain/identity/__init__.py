...
class Organization(AggregateRoot):
    ...
    def __post_init__(self) -> None:
        ...
        self.record_event(OrgCreated(org_id=self.id, name=self.name, plan=self.plan.value))
    ...
    def upgrade_plan(self, new_plan: PlanType) -> None:
        ...
        if order.index(new_plan) <= order.index(self.plan):
            raise DomainError(f"Cannot upgrade {self.plan.value} → {new_plan.value} (not upward)")
        from_plan = self.plan
        self.plan = new_plan
        self.record_event(
            OrgUpgraded(
