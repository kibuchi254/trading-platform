...
from platform.core.security import (
    generate_api_key, issue_token_pair, verify_password,
)
...
async def refresh(req: RefreshRequest) -> TokenResponse:
    ...
    pair = issue_token_pair(
        user_id=UUID(str(claims["sub"])), org_id=UUID(str(claims["org"])),
...
async def create_api_key(
    ...
    api_key = APIKey(
        org_id=user.org_id, user_id=user.user_id, name=name,
        key_prefix=raw[:8], key_hash=hashed, scopes=[user.role],
    )
    db.add(api_key)
    await db.commit()
    ...
    return APIKeyOut(
        ...
        raw_key=raw, scopes=api_key.scopes,
    )
