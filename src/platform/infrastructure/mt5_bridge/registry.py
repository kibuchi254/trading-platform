    ...
    async def select_for_symbol(
        ...
        async with self._lock:
            candidates = [
                ...
                if symbol in t.symbols and t.is_alive(self._heartbeat_timeout)
            ]
        if not candidates:
            return None
        if preferred_broker:
    ...
    async def start_watcher(self) -> None:
        if self._watcher_task is None or self._watcher_task.done():
            self._watcher_task = asyncio.create_task(
                self._watch_loop(), name="bridge-heartbeat-watcher"
            )
                ...
                async with self._lock:
                    for tid, rec in list(self._terminals.items()):
                        ...
                        if elapsed > self._heartbeat_timeout * 2:
                            _log.warning(
                                "terminal_offline_no_heartbeat", terminal_id=tid, elapsed_s=elapsed
                            )
