...
def _bar(close: float, *, ts: datetime, high: float | None = None,
         low: float | None = None, is_closed: bool = True) -> Bar:
    h = high if high is not None else close + 0.5
    l = low if low is not None else close - 0.5
    return Bar(
        symbol="XAUUSD", timeframe="M15", ts=ts,
        open=close, high=h, low=l, close=close, volume=1.0, is_closed=is_closed,
    )
...
async def test_no_signal_for_incomplete_bar() -> None:
    ...
    bar = _bar(100.0, ts=datetime(2026, 1, 1, tzinfo=timezone.utc), is_closed=False)
    assert await strat.on_bar(bar, _ctx()) is None
...
async def test_donchian_breakout_above_upper_emits_buy() -> None:
    ...
    sig = await strat.on_bar(
        _bar(105.0, ts=base + timedelta(minutes=15 * 10),
...
async def test_donchian_breakout_below_lower_emits_sell() -> None:
    ...
    sig = await strat.on_bar(
        _bar(95.0, ts=base + timedelta(minutes=15 * 10),
             high=96.0, low=94.0),
        _ctx(),
    )
...
async def test_breakout_requires_consecutive_confirmation_bars() -> None:
    ...
    await strat.on_bar(_bar(105.0, ts=base + timedelta(minutes=15 * 10),
                            high=106.0, low=104.0), _ctx())
    await strat.on_bar(_bar(100.0, ts=base + timedelta(minutes=15 * 11),
                            high=101.0, low=99.0), _ctx())
    # Two more above — should NOT trigger (only 2 consecutive after reset).
    sig = await strat.on_bar(_bar(105.0, ts=base + timedelta(minutes=15 * 12),
                                  high=106.0, low=104.0), _ctx())
    sig2 = await strat.on_bar(_bar(105.0, ts=base + timedelta(minutes=15 * 13),
                                   high=106.0, low=104.0), _ctx())
...
async def test_bollinger_mode_uses_bands_when_enabled() -> None:
    ...
    strat = BreakoutStrategy(channel_period=10, use_bollinger=True,
                             bb_std=2.0, confirmation_bars=1, min_strength=0.0)
    base = datetime(2026, 1, 1, tzinfo=timezone.utc)
    ...
    sig = await strat.on_bar(
        _bar(110.0, ts=base + timedelta(minutes=15 * 10),
...
async def test_min_strength_filters_breakout_signals() -> None:
    ...
    sig = await strat.on_bar(
        _bar(101.5, ts=base + timedelta(minutes=15 * 10),
