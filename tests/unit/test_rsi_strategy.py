...
async def test_no_signal_with_insufficient_data() -> None:
    ...
    bar = Bar(
        symbol="XAUUSD", timeframe="M15", ts=datetime.now(timezone.utc),
        open=100, high=101, low=99, close=100, volume=1.0, is_closed=True,
    )
...
async def test_emits_buy_on_oversold_recovery() -> None:
    ...
    for i, p in enumerate(prices):
        bar = Bar(
            symbol="XAUUSD", timeframe="M15", ts=base + timedelta(minutes=i * 15),
            open=p, high=p + 1, low=p - 1, close=p, volume=1.0, is_closed=True,
        )
    ...
    assert any(s.side == "buy" for s in signals), "Expected at least one BUY signal on oversold recovery"
